...
flickrshow = function(target, settings) {
  ...
  _.animate = function(element, property, endValue, speed, identifier) {
    ...
    return _.constants.intervals[identifier] = window.setInterval(execute, speed / 1.5);
  };
  _.onClickLeft = function() {
    ...
    if (typeof _.settings.onMove === 'function') {
      return _.settings.onMove(_.elements.images.childNodes[_.constants.imageCurrent].childNodes[0]);
    }
  };
  _.onClickPlay = function() {
    ...
    if (_.constants.isPlaying === false) {
      ...
      if (typeof _.settings.onPause === 'function') {
        return _.settings.onPause(_.elements.images.childNodes[_.constants.imageCurrent].childNodes[0]);
      }
    }
  };
  ...
  _.onLoadImage = function(event) {
    ...
    if (_.constants.imageLoaded === _.constants.imageTotal) {
      ...
      if (typeof _.settings.onLoad === 'function') {
        return _.settings.onLoad();
      }
    }
  };
  _.onLoadJson = function(event) {
    var areaM, areaS, areaT, areaZ, i, img, li, photo, _len, _len2, _ref, _ref2, _results;
    ...
    for (i = 0, _len2 = _ref2.length; i < _len2; i++) {
      ...
      _results.push(_.addEvent(img, 'load', _.onLoadImage));
    }
    return _results;
  };
  _.onLoadWindow = function(event) {
    ...
    return (document.getElementsByTagName('head')[0] || document.getElementsByTagName('body')[0]).appendChild(_.elements.script);
  };
  _.hideButtons = function() {
    ...
    return _.animate(_.elements.buttons, 'top', _.elements.target.offsetHeight, _.constants.speed, 'buttons');
  };
  _.showButtons = function() {
    ...
    return _.animate(_.elements.buttons, 'top', _.elements.target.offsetHeight - 40, _.constants.speed, 'buttons');
  };
  _.toggleButtons = function() {
    if (_.constants.isButtonsOpen === true) {
      return _.hideButtons();
    } else {
      return _.showButtons();
    }
  };
  _.showFlickr = function() {
    ...
    return window.location = 'http://www.flickr.com/photos/' + img.getAttribute('data-flickr-owner') + '/' + img.getAttribute('data-flickr-photo_id') + '/';
  };
  _.showTitle = function() {
    ...
    return _.elements.buttons.childNodes[3].innerHTML = (_.constants.imageCurrent + 1) + '/' + _.constants.imageTotal + ' - ' + img.getAttribute('data-flickr-title');
  };
  _.constants = {
